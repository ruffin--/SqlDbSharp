...
MVED#'s goal is to be a simplest-case, embeddable, crossplaform datastore written in C#. The [ongoing] emphasis is on a laconic, yet easy to understand and maintain, codebase that enables the serialization of data to files using a[n extremely small] subset of ANSI SQL.
...
To help ensure crossplatform usability, It has been written on both Visual Studio on Windows and MonoDevelop on Mac OS X.
...
As stated before, MVED#'s internal structure is designed to make it easy to visualize tables as raw hexadecimal values.  A very simple reader/visualizer is included in the MVED# repository or can be downloaded separately ([Windows](http://rufwork.com/code/mved/HexReader.zip)).
...
---
#####This may be the longest README.md on GitHub, at no extra charge. 